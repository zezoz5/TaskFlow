...
public class TaskItemController(ITaskItemService service) : ControllerBase
{
    ...
    [HttpGet]
    public async Task<IActionResult> GetAllTasks([FromRoute] Guid workspaceId, [FromRoute] Guid projectId, [FromQuery] TaskQueryParamsDto queryDto)
    {
        var userId = User.FindFirstValue(JwtRegisteredClaimNames.Sub);
        if (userId == null) return Unauthorized();

        var tasks = await _service.GetAllTasksAsync(userId, workspaceId, projectId, queryDto);
